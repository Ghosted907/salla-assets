...
(() => {
  ...
  const css = `
    ...
    /* ===== شريط الآراء المتحرك ===== */
    .jf-ticker{ position:absolute; left:1px; right:8%; bottom:20%; height:140px; background:rgba(239,234,226,.80); border-radius:16px; padding:10px 14px; overflow:hidden; z-index:2; display:flex; align-items:stretch; }
  ...
  function initTicker(){
    ...
    requestAnimationFrame(() => {
      ...
      let speed = isMobile ? 60 : 120; // px/sec (slower again)
      if (reduce) speed *= 0.6;
