(() => {
  ...
  /* ثبّت نسبة الصورة لتوحيد الإحداثيات عبر الأجهزة */
  .jf-hero{
    position:relative;display:block;width:100%;
    border-radius:40px;
    overflow:visible;                 /* لا نقص الشريط */
    aspect-ratio: 428 / 510;          /* من الصورة المرفقة */
  }
  ...
  @keyframes jf-marquee-up{
    ...
    100% { transform: translateY(var(--jf-distance)); } /* القيمة تُمرر px من JS */
  }
  ...
  /* ====== تركيب تلقائي داخل سلة ====== */
  function mount(){
    // بعد بانر التصنيفات المستهدف
    ...
    if (targetSection){
      if (!document.querySelector('.jf-container')) targetSection.insertAdjacentElement('afterend', buildBox());
      ensureHeroWrap(); attachTicker(); ensureGalleryModal();
      return true;
    }

    // بدائل
    const catBanner = Array.from(document.querySelectorAll('a.banner.banner--fixed, a.banner.banner--fixed.overflow-hidden'))
      .find(a => (a.getAttribute('href') || a.href || '').includes('jejetallow.com/redirect/categories/2058495926'));
    if (catBanner){
      if (!document.querySelector('.jf-container')) catBanner.insertAdjacentElement('afterend', buildBox());
      ensureHeroWrap(); attachTicker(); ensureGalleryModal();
      return true;
    }
    ...
    if (footer){
      if (!document.querySelector('.jf-container')) footer.parentNode.insertBefore(buildBox(), footer);
      ensureHeroWrap(); attachTicker(); ensureGalleryModal();
      return true;
    }
    return false;
  ...
  function lockScroll(){ document.documentElement.style.overflow = document.body.style.overflow = 'hidden'; }
  function unlockScroll(){ document.documentElement.style.overflow = document.body.style.overflow = ''; }
  function openGallery(){
    if (!GALLERY.length) return;
    galIndex = 0; showSlide();
    const dlg = document.getElementById('gallery-modal'); if (!dlg) return;
    dlg.showModal(); lockScroll();
  }
  ...
  /* ====== تريكر الآراء ====== */
  let resizeBound = false, resizeTimer = 0, ro = null;

  function attachTicker(){
    const root = document.querySelector('.jf-container'); if (!root) return;
    const hero = root.querySelector('.jf-hero') || root;
    if (hero.querySelector('.jf-ticker')) return;
    const wrap = document.createElement('div');
    wrap.className = 'jf-ticker'; wrap.dir = 'rtl'; wrap.setAttribute('aria-label','آراء العملاء');
    const inner = document.createElement('div');
    inner.id = 'jf-track'; inner.className = 'jf-track'; inner.setAttribute('aria-hidden','true');
    wrap.appendChild(inner); hero.appendChild(wrap);
    initTicker();
  }
  ...
  function initTicker(){
    ...
    // ضاعف المحتوى لحركة سلسة
    track.innerHTML = REVIEWS_TEXT.map(makeItem).join('') + REVIEWS_TEXT.map(makeItem).join('');

    // احسب المسافة والمدة بالبكسل
    requestAnimationFrame(() => {
      const distance = Math.ceil(track.scrollHeight / 2); // نصف الارتفاع لأن المحتوى مكرر
      ...
      track.style.setProperty('--jf-distance', `-${distance}px`); // CSS يتوقع px
    ...
    // مراقبة القياسات لإعادة الحساب
    if (!resizeBound){
      window.addEventListener('resize', () => {
        clearTimeout(resizeTimer);
        resizeTimer = setTimeout(resetTicker, 250);
      }, { passive:true });
      // ResizeObserver لخطوط/تغيّر محتوى
      if ('ResizeObserver' in window){
        ro = new ResizeObserver(() => resetTicker());
        ro.observe(track);
      }
      resizeBound = true;
    }
  ...
  function resetTicker(){
    const t = document.getElementById('jf-track'); if (!t) return;
    t.dataset.ready = '0';
